...
static void vmProcessWriteQueue(SQueueInfo *pInfo, STaosQall *qall, int32_t numOfMsgs) {
  ...
  vnodePreProcessWriteMsgs(pVnode->pImpl, pArray, &version);
  ...
  for (int32_t i = 0; i < numOfMsgs; i++) {
    ...
    int32_t code = vnodeProcessWriteMsg(pVnode->pImpl, pRpc, version, &pRsp);
    if (pRsp != NULL) {
...
static void vmProcessApplyQueue(SQueueInfo *pInfo, STaosQall *qall, int32_t numOfMsgs) {
  ...
  for (int32_t i = 0; i < numOfMsgs; ++i) {
    ...
    // todo
    SRpcMsg *pRsp = NULL;
    // (void)vnodeProcessWriteMsg(pVnode->pImpl, &pMsg->rpcMsg, &pRsp);
  }
}
