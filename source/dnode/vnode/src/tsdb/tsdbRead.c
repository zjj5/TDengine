...
//   int32_t        type;             // query type: retrieve all data blocks, 2. retrieve only last row, 3. retrieve
//   direct prev|next rows SDFileSet*     pFileGroup; SFSIter        fileIter; SReadH         rhelper; STableBlockInfo*
//   pDataBlockInfo; SDataCols     *pDataCols;        // in order to hold current file data block int32_t allocSize; //
//   allocated data block size SArray        *defaultLoadColumn;// default load column SDataBlockLoadInfo
//   dataBlockLoadInfo; /* record current block load information */ SLoadCompBlockInfo compBlockLoadInfo; /* record
//   current compblock information in SQueryAttr */
...
// STsdbReadHandle* pTsdbReadHandle); static int32_t tsdbCheckInfoCompar(const void* key1, const void* key2);
...
tsdbReaderT* tsdbQueryTables(STsdb* tsdb, STsdbQueryCond* pCond, STableGroupInfo* groupList, uint64_t qId,
  ...
  //   taosArrayGetSize(pTsdbReadHandle->pTableCheckInfo),
...
//   &pTable); if (pTsdbReadHandle->pTableCheckInfo == NULL) {
// //    tsdbCleanupReadHandle(pTsdbReadHandle);
//     terrno = TSDB_CODE_TDB_OUT_OF_MEMORY;
//   }
...
// tsdbReaderT tsdbQueryLastRow(STsdb *tsdb, STsdbQueryCond *pCond, STableGroupInfo *groupList, uint64_t qId, uint64_t
// taskId) {
...
// tsdbReaderT tsdbQueryRowsInExternalWindow(STsdb *tsdb, STsdbQueryCond* pCond, STableGroupInfo *groupList, uint64_t
// qId, uint64_t taskId) {
...
//   bool imemEmpty = (pCheckInfo->iiter == NULL) || (pCheckInfo->iiter != NULL &&
//   !tSkipListIterNext(pCheckInfo->iiter)); if (memEmpty && imemEmpty) { // buffer is empty
//     return false;
//   }
...
//   if (ASCENDING_TRAVERSE(pTsdbReadHandle->order)) {
//     assert(pCheckInfo->lastKey <= pTsdbReadHandle->window.ekey && pTsdbReadHandle->window.skey <=
...
//   (int)(QH_GET_NUM_OF_COLS(pTsdbReadHandle)), true); if (ret != TSDB_CODE_SUCCESS) {
//     int32_t c = terrno;
...
//   1970-01-01T00:00:00Z if(pBlock->keyFirst < 0 && colIds[0] == PRIMARYKEY_TIMESTAMP_COL_ID) {
...
//             pTsdbReadHandle, slotIndex, pBlock->keyFirst, pBlock->keyLast, pBlock->numOfRows,
//             pTsdbReadHandle->idStr);
//   return terrno;
// }

// static int32_t getEndPosInDataBlock(STsdbReadHandle* pTsdbReadHandle, SDataBlockInfo* pBlockInfo);
// static int32_t doCopyRowsFromFileBlock(STsdbReadHandle* pTsdbReadHandle, int32_t capacity, int32_t numOfRows, int32_t
// start, int32_t end); static void moveDataToFront(STsdbReadHandle* pTsdbReadHandle, int32_t numOfRows, int32_t
// numOfCols); static void doCheckGeneratedBlockRange(STsdbReadHandle* pTsdbReadHandle); static void
// copyAllRemainRowsFromFileBlock(STsdbReadHandle* pTsdbReadHandle, STableCheckInfo* pCheckInfo, SDataBlockInfo*
// pBlockInfo, int32_t endPos);

// static int32_t handleDataMergeIfNeeded(STsdbReadHandle* pTsdbReadHandle, SBlock* pBlock, STableCheckInfo*
// pCheckInfo){
...
//       TSKEY maxKey = ASCENDING_TRAVERSE(pTsdbReadHandle->order)? (binfo.window.skey - step):(binfo.window.ekey -
//       step); cur->rows = tsdbReadRowsFromCache(pCheckInfo, maxKey, pTsdbReadHandle->outputCapacity, &cur->win,
//       pTsdbReadHandle); pTsdbReadHandle->realNumOfRows = cur->rows;
...
//     } else {
//       tsdbDebug("%p create data block from remain file block, brange:%"PRId64"-%"PRId64", rows:%d, total:%d,
//       lastKey:%"PRId64", %s",
...
// static int32_t loadFileDataBlock(STsdbReadHandle* pTsdbReadHandle, SBlock* pBlock, STableCheckInfo* pCheckInfo, bool*
// exists) {
//   SQueryFilePos* cur = &pTsdbReadHandle->cur;
//   int32_t code = TSDB_CODE_SUCCESS;
//   bool asc = ASCENDING_TRAVERSE(pTsdbReadHandle->order);
...
//         cur->pos = binarySearchForKey(pTsCol->cols[0].pData, pBlock->numOfRows, pCheckInfo->lastKey,
//         pTsdbReadHandle->order);
//       } else {
...
//     for(int32_t i = 0; i < numOfCols; ++i) {
//       SColumnInfoData* pColInfo = taosArrayGet(pTsdbReadHandle->pColumns, i);
//       memmove((char*)pColInfo->pData, (char*)pColInfo->pData + emptySize * pColInfo->info.bytes, numOfRows *
//       pColInfo->info.bytes);
//     }
//   }
// }

// static void getQualifiedRowsPos(STsdbReadHandle* pTsdbReadHandle, int32_t startPos, int32_t endPos, int32_t
// numOfExisted,
...
//     assert(cur->win.skey == ((TSKEY*)pColInfoData->pData)[0] && cur->win.ekey ==
//     ((TSKEY*)pColInfoData->pData)[cur->rows-1]);
//   } else {
...
//   TSKEY* tsArray = pCols->cols[0].pData;
//   assert(pCols->numOfRows == pBlock->numOfRows && tsArray[0] == pBlock->keyFirst && tsArray[pBlock->numOfRows-1] ==
//   pBlock->keyLast);
...
//         pCheckInfo->tableId, pSchema1, pSchema2, true); numOfRows += 1; if (cur->win.skey == TSKEY_INITIAL_VAL) {
//           cur->win.skey = key;
//         }
...
//           mergeTwoRowFromMem(pTsdbReadHandle, pTsdbReadHandle->outputCapacity, numOfRows, row1, row2, numOfCols,
//           pCheckInfo->tableId, pSchema1, pSchema2, forceSetNull); numOfRows += 1; if (cur->win.skey ==
//           TSKEY_INITIAL_VAL) {
//             cur->win.skey = key;
//           }
...
//         numOfRows = doCopyRowsFromFileBlock(pTsdbReadHandle, pTsdbReadHandle->outputCapacity, numOfRows, qstart,
//         qend); pos += (qend - qstart + 1) * step;
...
//       ||
//        ((pos < endPos || cur->lastKey < pTsdbReadHandle->window.ekey) &&
//        !ASCENDING_TRAVERSE(pTsdbReadHandle->order)));
...
int32_t tsdbGetFileBlocksDistInfo(tsdbReaderT* queryHandle, STableBlockDistInfo* pTableBlockInfo) {
  ...
  //     // current file are not overlapped with query time window, ignore remain files
  //     if ((ascTraverse && win.skey > pTsdbReadHandle->window.ekey) || (!ascTraverse && win.ekey <
  //     pTsdbReadHandle->window.ekey)) {
  //       tsdbUnLockFS(REPO_FS(pTsdbReadHandle->pTsdb));
  //       tsdbDebug("%p remain files are not qualified for qrange:%" PRId64 "-%" PRId64 ", ignore, %s",
  //       pTsdbReadHandle,
  //                 pTsdbReadHandle->window.skey, pTsdbReadHandle->window.ekey, pTsdbReadHandle->idStr);
  //       pTsdbReadHandle->pFileGroup = NULL;
  //       break;
...
//     if ((key > maxKey && ASCENDING_TRAVERSE(pTsdbReadHandle->order)) || (key < maxKey &&
//     !ASCENDING_TRAVERSE(pTsdbReadHandle->order))) {
//       tsdbDebug("%p key:%"PRIu64" beyond qrange:%"PRId64" - %"PRId64", no more data in buffer", pTsdbReadHandle, key,
//       pTsdbReadHandle->window.skey,
//                 pTsdbReadHandle->window.ekey);
...
// //      return false;
// //    }
//     mergeTwoRowFromMem(pTsdbReadHandle, pTsdbReadHandle->outputCapacity, 0, pRow, NULL, numOfCols,
//     pCheckInfo->tableId, NULL, NULL, true); taosMemoryFreeClear(pRow);
...
bool tsdbNextDataBlock(tsdbReaderT pHandle) {
  ...
  //     pTsdbReadHandle->idStr); return false;
  //   }
...
// //
// //  SArray* psTable = NULL;
// //  pSecQueryHandle->pTableCheckInfo = createCheckInfoFromCheckInfo(pCurrent, pSecQueryHandle->window.skey,
// &psTable);
...
int32_t tsdbRetrieveDataBlockStatisInfo(tsdbReaderT* pTsdbReadHandle, SDataStatis** pBlockStatis) {
  ...
  //   assert((c->slot >= 0 && c->slot < pHandle->numOfBlocks) || ((c->slot == pHandle->numOfBlocks) && (c->slot ==
  //   0)));
...
int32_t tsdbQuerySTableByTagCond(void* pMeta, uint64_t uid, TSKEY skey, const char* pTagCond, size_t len,
  ...
  //     tsdbError("%p query normal tag not allowed, uid:%" PRIu64 ", TID:0x%"PRIx64" QID:0x%"PRIx64, pMeta, uid,
  //     taskId, reqId); terrno = TSDB_CODE_OPS_NOT_SUPPORT; //basically, this error is caused by invalid sql issued by
  //     client goto _error;
  //   }
...
//       pTsdbReadHandle, pCost->headFileLoad, pCost->headFileLoadTime, pCost->statisInfoLoadTime, pCost->blockLoadTime,
//       pCost->checkForNextTime, pTsdbReadHandle->idStr);
...
// static void applyFilterToSkipListNode(SSkipList *pSkipList, tExprNode *pExpr, SArray *pResult, SExprTraverseSupp
// *param) {
...
//   } else {
//     iter = tSkipListCreateIterFromVal(pSkipList, (char*)(cond.end ? cond.end->v: NULL), pSkipList->type,
//     TSDB_ORDER_DESC);
//   }
...
// static void queryIndexlessColumn(SSkipList* pSkipList, tQueryInfo* pQueryInfo, SArray* res, __result_filter_fn_t
// filterFp) {
//   SSkipListIterator* iter = tSkipListCreateIter(pSkipList);
...
//     assert(pLeft->nodeType == TSQL_NODE_COL && (pRight->nodeType == TSQL_NODE_VALUE || pRight->nodeType ==
//     TSQL_NODE_DUMMY));
...
//                                 && pQueryInfo->optr != TSDB_RELATION_MATCH && pQueryInfo->optr !=
