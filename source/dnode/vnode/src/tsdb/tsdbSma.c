...
static int32_t tsdbTSmaDataSplit(STSmaWriteH *pSmaH, STSma *param, STSmaData *pData, int32_t days, int32_t nOffset,
  ...
  for (int32_t n = nOffset + 1; n < pData->numOfSmaBlocks; ++n) {
...
int32_t tsdbInsertTSmaDataImpl(STsdb *pTsdb, STSma *param, STSmaData *pData) {
  ...
  if (pData->numOfSmaBlocks <= 0 || pData->numOfColIds <= 0 || pData->dataLen <= 0) {
    TASSERT(0);
  ...
  if (minFid == maxFid) {  
    ...
    tsdbInsertTSmaDataSection(&tSmaH, pData, 0, pData->numOfSmaBlocks);
    ...
    do {
      ...
      if (tFid == maxFid) {
        tsdbSetTSmaDataFile(&tSmaH, param, pData, storageLevel, tFid);
        tsdbInsertTSmaDataSection(&tSmaH, pData, nOffset, pData->numOfSmaBlocks - nOffset);
        break;
      }
    } while (true);
...
int32_t tsdbInsertRSmaDataImpl(STsdb *pTsdb, SRSma *param, STSmaData *pData) {
  ...
  int32_t nSmaBlocks = pData->numOfSmaBlocks;
  ...
  if (minFid == maxFid) {
    ...
    int32_t tmpFid = 0;
    int32_t step = 0;
    for (int32_t n = 0; n < pData->numOfSmaBlocks; ++n) {
    }
    tsdbInsertTSmaDataSection(&tSmaH, pData, colDataLen, nSmaBlocks);
    // TODO:tsdbEndTSmaCommit();
  } else {
