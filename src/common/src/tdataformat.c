...
STSRow tdMemRowDup(STSRow *row) {
  STSRow trow = malloc(memRowTLen(row));
  if (trow == NULL) return NULL;
  ...
  for (int i = 0; i < pDataCols->numOfCols; i++) {
    ...
    if (keepData) {
      if (pDataCols->cols[i].lenXYZ > 0) {
        ...
        memcpy(pRet->cols[i].pBitmap, pDataCols->cols[i].pBitmap, (size_t)TD_BITMAP_BYTES(pDataCols->numOfRows));
      }
    }
  }
...
#if 0
static void tdAppendDataRowToDataCol(STpRow *row, STSchema *pSchema, SDataCols *pCols, bool forceSetNull,
                                     int rowOffset) {
