        ...
        for i in range(2):
            try:
                ...
                for i in range(2):
                    ...
                    #sql = "select min(q_smallint_null) like max(q_smallint) from table_2 where q_smallint is null and  q_tinyint != 128 and  q_binary nmatch '123_' and  (q_bool = true or q_bool = false) session(ts,8d);"
                    ...
                    sql = "select round(q_double_null) != round(q_double_null) from table_2 "
                    tdSql.query(sql)
