            ...
            try:
                ...
                for i in range(2,len(regular_where[2])+1):
                    ...
                    for q_where in q_where:
                        ...
                        sql2 = "select %s from %s where %s %s %s order by ts" %(func,table,q_where,q_like_match,q_in_where)
                        tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql2 ,1,1)
                        cur1.execute(sql2)

                        sql2 = "select * from (select %s from %s where %s %s %s order by ts)" %(func,table,q_where,q_like_match,q_in_where)
                        tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql2 ,1,1)
                        cur1.execute(sql2)

                        sql2 = "select %s from (select * from %s) where %s %s %s order by ts" %(func,table,q_where,q_like_match,q_in_where)
                        tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql2 ,1,1)
                        cur1.execute(sql2)
                
                regular_where = tdWhere.regular_where()
                sql1 = 'select %s from %s order by ts desc;'  % (func,table)
                for i in range(2,len(regular_where[2])+1):
                    q_where = list(combinations(regular_where[2],i))
                    for q_where in q_where:
                        q_where = str(q_where).replace("(","").replace(")","").replace("'","").replace("\"","").replace(",","")
                        ...
                        sql2 = "select %s from %s where %s %s %s order by ts desc" %(func,table,q_where,q_like_match,q_in_where)
                        tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql2 ,1,1)
                        cur1.execute(sql2)

                        sql2 = "select * from (select %s from %s where %s %s %s order by ts desc)" %(func,table,q_where,q_like_match,q_in_where)
                        tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql2 ,1,1)
                        cur1.execute(sql2)

                        sql2 = "select %s from (select * from %s) where %s %s %s order by ts desc" %(func,table,q_where,q_like_match,q_in_where)
                        tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql2 ,1,1)
                        cur1.execute(sql2)
                ...
                for i in range(2,len(regular_where[2])+1):
                    ...
                    for q_where in q_where:
                        ...
                        sql2 = "select %s from %s where %s %s %s order by ts limit 10" %(func,table,q_where,q_like_match,q_in_where)
                        tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql2 ,1,1)
                        cur1.execute(sql2)

                        sql2 = "select * from (select %s from %s where %s %s %s order by ts limit 10)" %(func,table,q_where,q_like_match,q_in_where)
                        tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql2 ,1,1)
                        cur1.execute(sql2)

                        sql2 = "select %s from (select * from %s) where %s %s %s order by ts limit 10" %(func,table,q_where,q_like_match,q_in_where)
                        tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql2 ,1,1)
                        cur1.execute(sql2)
                ...
                # regular_where = tdWhere.regular_where()
                # sql1 = 'select %s from %s limit 10 offset 5;'  % (func,table)
                # for i in range(2,len(regular_where[2])+1):
                #     q_where = list(combinations(regular_where[2],i))
                #     for q_where in q_where:
                #         q_where = str(q_where).replace("(","").replace(")","").replace("'","").replace("\"","").replace(",","")
                ...
                #         q_in_where = regular_where[4]
                        ...
                        # sql2 = "select %s from %s where %s %s %s order by ts limit 10 offset 5" %(func,table,q_where,q_like_match,q_in_where)
                        # tdCreateData.dataequal('%s' %sql1 ,1,1,'%s' %sql2 ,1,1)
                        # cur1.execute(sql2)
